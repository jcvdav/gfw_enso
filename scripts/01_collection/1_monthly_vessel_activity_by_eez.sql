################################################################
################ monthly_vessel_activity_by_eez ################
################################################################
# This query produces a panel showing the activity fovessel_info
...
WITH
  ...
  IF
    (ARRAY_LENGTH(regions.eez) = 0,
      ...
      hours
  FROM
    `world-fishing-827.gfw_research.pipe_v20200203_fishing`
  WHERE
    ssvid IN (
    SELECT
      DISTINCT(ssvid)
...
SELECT
  ...
  fishing_hours * best_engine_power_kw AS fishing_kilowats,
  nonfishing_hours * best_engine_power_kw AS nonfishing_kilowats
FROM
  panel