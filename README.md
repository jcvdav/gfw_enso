# GFW_ENSO


## Repository structure 

```
-- data
   |__all_indices.csv
   |__asian_countries_foreign_fishing_year_month_season_vessel.rds
   |__cor_sst_nino34_2deg.csv
   |__eez_sst_nino34_cor_df.csv
   |__foreign_fishing_year_month_season_vessel.rds
   |__gridded_ff_by_gear_country.rds
   |__masked_aggregated_raster.rds
   |__model_data_ll.rds
   |__model_data_ps.rds
   |__month_rasters.rds
   |__raster_stack.rds
   |__raster_stack_masked.rds
   |__rasterized_wdpa.nc
   |__sst.csv
   |__sst_df_whole.rds
   |__sst_nino3_df.csv
   |__sst_nino3_df.rds
   |__sst_nino34_cor_df.rds
   |__sst_nino34_df.rds
-- desktop.ini
-- docs
   |__CHN_ff.html
   |__CHN_ff.Rmd
   |__CHN_ff_for_Cody.pdf
   |__CHN_ff_for_Cody.Rmd
   |__CHN_ff_for_Cody_top10.pdf
   |__CHN_ff_for_Cody_top10.Rmd
   |__DID.tex
   |__nar.bst.txt
   |__nature.bst
   |__Oremus_Villasenor-Derbez.pdf
   |__Oremus_Villasenor-Derbez.Rmd
   |__Oremus_Villasenor-Derbez.tex
   |__Oremus_Villasenor-Derbez_files
   |__plos2015.bst
   |__references_temp.bib
   |__test.html
   |__test.pdf
   |__test.Rmd
-- gfw_enso.Rproj
-- git_repo
-- packrat
   |__init.R
   |__lib
   |__lib-ext
   |__lib-R
   |__packrat.lock
   |__packrat.opts
   |__src
-- raw_data
   |__ff_by_gear_country.rds
   |__foreign_fishing_year_month_season_vessel.rds
   |__GFW
   |__gridded_ff_by_gear_country.rds
   |__indices
   |__spatial
-- README.md
-- scripts
   |__analyses.R
   |__analyses_by_region.R
   |__collection
   |__content
   |__cor_eez_sst_nino34.R
   |__cor_ff_nino3.R
   |__cor_sst_nino34.R
   |__descriptive_figures.R
   |__download_foreign_fishing_year_month_season_vessel.R
   |__download_gridded_ff_by_gear_country.R
   |__foreign_fishing_year_month_season_vessel.sql
   |__GFW_raster.R
   |__gridded_ff_by_gear_country.sql
   |__join_all_indices.R
   |__processing
   |__setup.R
   |__teleconnected.R
   |__TEST_illegal_fishing_effort_CHN.sql
-- slides
   |__libs
-- writing
   |__AERE_abstract.bbl
   |__AERE_abstract.pdf
   |__AERE_abstract.Rmd
   |__AERE_abstract.synctex.gz
   |__AERE_abstract.tex
   |__AERE_abstract_files
   |__img
   |__paper.aux
   |__paper.bbl
   |__paper.blg
   |__paper.log
   |__paper.pdf
   |__paper.synctex.gz
   |__paper.tex
   |__POLS207
   |__references.bib
   |__tab
```

--------- 

<a href="https://orcid.org/0000-0003-1245-589X" target="orcid.widget" rel="noopener noreferrer" style="vertical-align:top;"><img src="https://orcid.org/sites/default/files/images/orcid_16x16.png" style="width:1em;margin-right:.5em;" alt="ORCID iD icon">orcid.org/0000-0003-1245-589X</a>
