...
![Spatial correlation between SST and El Nino (as identified by ENSO3.4 anomaly index). The colors indicate the number of months in a year for which SST was significantly correlated to ENSO3.4 anomaly. Isoclines outline the 3-month and 5-month correlation criteria (see above). This image can be interpreted as "For areas with color x, SST was explained by NINO3.4 at least n months in a year".](img/teleconnections_by_months.png)
...
panel <- readRDS(file = here("data", "eez_fishing_effort_treatment_panel.rds")) %>% 
  ...
  select(year, month, nino34anom, eez_iso3, status_c, status, foreign_hours, local_hours, total_hours, prop_hours_foreign, area) %>%
...
There are some things to consider in order to estimate the ENSO-sensitivity of fishing hours for TE and WE EEZs. The addition of satellites to the constellation used to detect AIS leads to better detection of fishing activity, and may bias results. Before June, 2014 there were only three satellites, then 6 until December 2015, and then 10 until January 2017. I don't have the exact number now, but detection of fishing activity levels-off at 6 / 7 satellites. I therefore include four dummies for each different "period" of satellite coverage as: 01/2012 - 06/2014, 06/2014 - 12/2015, 12/2015 - 01/2017, 01/2017 - present (Fig 5). If we didn't have this information, a linear trend that captures slow increases in overall fishing and ENSO could be used, as HMC do in their estimation of ECR and NINO effects.

Each EEZ is likely to be subject to different levels of fishing (both local and foreign) due to the "quality" of the fishing grounds in them, as well as levels of enforcement and fisheries management overall. For this reason, it is sensible to include EEZ-level fixed effects that can account for this unobserved characteristics. Additionally, there is likely to be within-EEZ correlation and heteroskedasticity in fishing activity, so standard errors should also be clustered at the EEZ level. I use the "stata-like" variance estimator, which is similar to Cluster-Robust (*i.e.* CR0) but includes a finite-sample correction of the form: $\frac{N-1}{N-K}\frac{S}{S-1}$ with N for sample size, S for number of clusters, and K for the number of estimated coefficients (my clusters do not approach infinity: 75 EEZs are TE, 85 are WA).

The measures of ENSO indices are obtained after the event is realized (*i.e.* at the end of the month). Fishing operations are likely driven by their immediate exposure (*i.e.* "If fishing grounds are not right now, I will not fish here now"). I use contemporary NINO3.4 indices to characterize the variation to which vessels were exposed in a given month. The indices show significant serial correlation (Fig 6), and including lags would not change results dramatically. We are interested in the number of additional hours explained by ENSO, so I will include NINO3.4 with a linear trend only.

My estimation model therefore contains:

- A linear trend for contemporary NINO3.4 indices, captured by $\beta$
...
The inclusion / exclusion of satellite dummies shows that some confounding exists ($\beta_3$ can vary by up to 27\%). As expected, the NINO34 index, which is time-varying, captures the effect of addition of satelites. This is an important note, because addition of satellites seems to confound the $\beta_3$ estimate. However, the estimates for $\beta_0$ (intercept) and $\beta_2$ (status) are more robust to the inclussion of this dummies.
